 ...
 */
export function extractCriticalInfo(contents) {
    const criticalInfo = [];
    for (const content of contents) {
        ...
        const filePathMatch = content.match(/(?:created|modified|edited|updated|deleted|wrote|writing)\s+(?:file\s+)?['"`]?([^\s'"`\n]*(?:\.[a-zA-Z0-9]+|Makefile|Dockerfile|README|LICENSE|CHANGELOG))['"`]?/gi);
        if (filePathMatch) {
        ...
        const searchMatch = content.match(/(?:found|located|match(?:es)?)\s+(?:in|at)\s+['"`]?([^\s'"`\n]+(?:\.[a-zA-Z0-9]+))['"`]?(?:\s*[:\(]\s*(?:line\s*)?\d+)?/gi);
        if (searchMatch) {
 ...
 */
export function findMostCommonTool(results) {
 ...
 */
export function detectStuckPattern(recentResults, config) {
    ...
    if (allFailed) {
        const errors = [...new Set(lastResults.map(r => r.error).filter(Boolean))];
